...
export default class Yatzy {
  ...
  static score_pair(...fiveDices: FiveDices): number {
    const highestNumber = matchingDices(...fiveDices).reduce(
  ...
  static two_pair(...fiveDices: FiveDices): number {
    ...
    fiveDices.forEach((value: Dice) => {
      counts[dieValueToIndex(value)]++;
    });
    return (
      counts
        .map((value, index) =>
          value >= 2 ? (indexToDieValue(index) as number) : 0
        )
  ...
  static four_of_a_kind(...fiveDices: FiveDices): number {
    ...
    return (
      counts
        .map((value, index) =>
          value >= 4 ? (indexToDieValue(index) as number) : 0
        )
        .reduce((acc, value) => acc + value, 0) * 4
    );
  }
  ...
  static three_of_a_kind(...fiveDices: FiveDices): number {
    ...
    return (
      counts
        .map((value, index) =>
          value >= 3 ? (indexToDieValue(index) as number) : 0
        )
  ...
  static fullHouse(...fiveDices: FiveDices): number {
    const counts = getMapOfCounts(fiveDices);
    return Array.from(counts.entries())
      .filter((mapEntry, index, array): boolean =>
        array.some((mapEntry): boolean => mapEntry[1] === 2))
      .filter((mapEntry, index, array): boolean =>
        array.some((mapEntry): boolean => mapEntry[1] === 3))
      .map((mapEntry) => mapEntry[0] * mapEntry[1])
      .reduce((acc, value) => acc + value, 0);
  }
}

function getMapOfCounts(fiveDices: FiveDices) {
  const counts = new Map<Dice, Count>()
  fiveDices.forEach((value) => {
    counts.set(value, ((counts.get(value) ?? 0) + 1) as Count)
  })
  return counts
}
...
function indexToDieValue(index: number): Dice {
  assert(index >= 0 && index <= 5)
  return (index + 1) as Dice
}
